---
title: "Change of energy prices in EU from 2019 to 2022!"
...
## 4.Change of energy prices in 2019
```{r fig01}
prices_2019 <- prc_hicp_mmor %>%
  filter(coicop %in% c("NRG"), time > "2018-12-31" & time < "2020-01-01") %>% 
  group_by(geo) %>% summarise(sm = sum(values))
df_2019<-eurasia %>% 
  inner_join(prices_2019, by = c("iso_a2" = "geo"))
df_2019 %>% select(name, iso_a2, sm, geometry) %>% 
  ggplot() +
...
## 5.Change of energy prices in 2020
```{r fig02}
prices_2020 <- prc_hicp_mmor %>%
  filter(coicop %in% c("NRG"), time > "2019-12-31" & time < "2021-01-01") %>% 
  group_by(geo) %>% summarise(sm = sum(values))
df_2020<-eurasia %>% 
...
## 6.Change of energy prices in 2021
```{r fig03}
prices_2021 <- prc_hicp_mmor %>%
  filter(coicop %in% c("NRG"), time > "2020-12-31" & time < "2022-01-01") %>% 
  group_by(geo) %>% summarise(sm = sum(values))
df_2021<-eurasia %>% 
...
## 7.Change of energy prices in 2022 (January to October)
```{r fig04}
prices_2022 <- prc_hicp_mmor %>%
  filter(coicop %in% c("NRG"), time > "2021-12-31") %>% 
  group_by(geo) %>% summarise(sm = sum(values))
df_2022<-eurasia %>% 
...
## 8.Cumulative change of energy prices from 2019 to 2022
```{r fig05}
prices_2019_2022 <- prc_hicp_mmor %>%
  filter(coicop %in% c("NRG"), time > "2018-12-31") %>% 
  group_by(geo) %>% summarise(sm = sum(values))
